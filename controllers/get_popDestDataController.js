        ...
        const query = {
            text: `
                ...
                LEFT JOIN (
                    SELECT 
                        *,
                        ROW_NUMBER() OVER(PARTITION BY "locationCityID" ORDER BY "locationScore" DESC) AS rn
                    FROM 
                        locations
                ) AS locations ON cities."cityID" = locations."locationCityID" AND locations.rn <= 5
                GROUP BY 
                    cities."cityID" 
                ORDER BY 
